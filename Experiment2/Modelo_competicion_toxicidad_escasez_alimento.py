#MODELO COMPETICION TOXICIDAD + ESCASEZ DE ALIMENTO
...
rows_columns=100
#Ratio de competicion (distancia por debajo de la cual se consideran vecinos a los agentes)
competition_ratio=5
...
#Tasa de secrecion de la sustancia toxica
secretion_rate=[[0.1,0.1],[0.1,0.2],[0.2,0.1]]
#Numero maximo (numero de vecinos en el cual la tasa de reproduccion es 0)
max_numbers=[40,80,160]
...
def initialize():
    ...
    global agents, toxin_1, toxin_2, species_1_number, species_2_number, total_number
    ...
    for i in range (number_agents):
        ...
        #El septimo atributo se refiere a la tasa de secrecion de la sustancia toxica
...
def observe():
    ...
    #Dibujamos en la grafica los alimentos y las sustancias toxicas
    ...
    matplotlib.pyplot.imshow(toxin_1,cmap=cmap1,vmin=0,vmax=10,alpha=1)
    matplotlib.pyplot.imshow(toxin_2,cmap=cmap2,vmin=0,vmax=10,alpha=0.6)
...
def update():
    ...
    #CONSUMO DE LAS SUSTANCIAS TOXICAS Y DE ALIMENTO
    #Ponemos valor a la variable consume_toxina_2
    #Solo la especie 1 puede consumir la sustancia toxica 2
    if species==1:
    ...
    #Solo la especie 2 puede consumir la sustancia toxica 1
    elif species==2:
        consume_toxina_2=False
    ...
    n=sum(1 for nb in agents if (ag.x-nb.x)**2+(ag.y-nb.y)**2 < competition_ratio**2)-1
    
    #El agente puede haber consumido la sustancia toxica o no haberlo hecho
    ...
    if consume_toxina_2==True or consume_toxina_1==True:
        if random()<(reproduction_rate-((reproduction_rate)*(n/max_number)))*(1-sum(1 for x in agents)//carrying_capacity):
            newborn=copy(ag) 
...
os.mkdir("/home/pedro/Simulaciones_modelo_competicion_toxicidad_ea")
os.mkdir("/home/pedro/Simulaciones_modelo_competicion_toxicidad_ea/Distribucion_espacial_inicial")
os.mkdir("/home/pedro/Simulaciones_modelo_competicion_toxicidad_ea/Distribucion_espacial_mitad")
os.mkdir("/home/pedro/Simulaciones_modelo_competicion_toxicidad_ea/Distribucion_espacial_final")
os.mkdir("/home/pedro/Simulaciones_modelo_competicion_toxicidad_ea/Evolucion_poblaciones")
os.mkdir("/home/pedro/Simulaciones_modelo_competicion_toxicidad_ea/Coordenadas_espaciales")
    ...
    for m in movement:
        for s in secretion_rate:
            for mn in max_numbers:
                for r in replicas:
                    combinaciones.append([p,m,s,mn,r])
...
for i in combinaciones:
    ...
    simulacion="p"+str(p)+"m"+str(m)+"s"+str(s)+"mn"+str(max_number)+"r"+str(r)
...
#Generamos una documento con las coordenadas de cada agente al final de la simulacion
    ...
    initialize()
    observe()
    os.chdir("/home/pedro/Simulaciones_modelo_competicion_toxicidad_ea/Distribucion_espacial_inicial")
    matplotlib.pyplot.savefig("modelo_competicion_toxicidad_inicio_"+str(simulacion)+".png")
    ...
    for j in range(number_steps):
        ...
        if j==(number_steps/2):
            observe()
            os.chdir("/home/pedro/Simulaciones_modelo_competicion_toxicidad_ea/Distribucion_espacial_mitad")
            matplotlib.pyplot.savefig("modelo_competicion_toxicidad_mitad_"+str(simulacion)+".png")
    
    observe()
    os.chdir("/home/pedro/Simulaciones_modelo_competicion_toxicidad_ea/Distribucion_espacial_final")
    matplotlib.pyplot.savefig("modelo_competicion_toxicidad_final_"+str(simulacion)+".png")
    
    os.chdir("/home/pedro/Simulaciones_modelo_competicion_toxicidad_ea/Coordenadas_espaciales")
    document=open("modelo_competicion_toxicidad_coordenadas_espaciales"+str(simulacion)+".txt","w")
    document.write("Especie" + '\t' + "Coordenadas X" + '\t' + "Coordenadas Y" + '\n')
    for ag in agents:
        document.write(str(ag.type) + '\t' + str(ag.x) + '\t' + str(ag.y) + '\n')
    document.close()
    
    os.chdir("/home/pedro/Simulaciones_modelo_competicion_toxicidad_ea/Evolucion_poblaciones")
    document=open("modelo_competicion_toxicidad_evolucion_poblaciones"+str(simulacion)+".txt","w")
