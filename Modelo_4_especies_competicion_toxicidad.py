#MODELO COMPETICION TOXICIDAD (4 ESPECIES)
...
def initialize():
    ...
    global agents, toxin_1, toxin_2, toxin_3, toxin_4
    ...
    toxin_3=zeros([rows_columns,rows_columns])
    toxin_4=zeros([rows_columns,rows_columns])
...
def observe():
    ...
    if len(species_1)>0:
        matplotlib.pyplot.scatter([ag.x for ag in species_1],[ag.y for ag in species_1], marker="o",color="#FFAB91")
    if len(species_2)>0:
        matplotlib.pyplot.scatter([ag.x for ag in species_2],[ag.y for ag in species_2], marker="o",color="#FF7043")
    if len(species_3)>0:
        matplotlib.pyplot.scatter([ag.x for ag in species_3],[ag.y for ag in species_3], marker="o",color="#F4511E")
    if len(species_4)>0:
        matplotlib.pyplot.scatter([ag.x for ag in species_4],[ag.y for ag in species_4], marker="o",color="#D84315")
    #Dibujamos en la grafica las sustancias toxicas
    color0 = matplotlib.colors.colorConverter.to_rgba('white')
    color1 = matplotlib.colors.colorConverter.to_rgba('#FFAB91')
    color2 = matplotlib.colors.colorConverter.to_rgba('#FF7043')
    color3 = matplotlib.colors.colorConverter.to_rgba('#F4511E')
    color4 = matplotlib.colors.colorConverter.to_rgba('#D84315')                                                      
    ...
    matplotlib.pyplot.imshow(toxin_2,cmap=cmap2,vmin=0,vmax=5,alpha=0.75)
    matplotlib.pyplot.imshow(toxin_3,cmap=cmap3,vmin=0,vmax=5,alpha=0.5)
    matplotlib.pyplot.imshow(toxin_4,cmap=cmap4,vmin=0,vmax=5,alpha=0.25)
...
def update():
    ...
    #CONSUMO DE SUSTANCIAS TOXICAS Y ALIMENTO
    #Como los arrays espaciales son independientes, en una misma posicion podriamos tener toxinas que provienen de especies distintas
    #A nosotros nos interesa que, a cada iteracion, un agente solo consuma una toxina, de manera que cuando ya ha consumido uno se deja de comprobar si existen de otro tipo
    #Para no establecer un orden de predominancia, hacemos que el orden sea aleatorio
    #La especie 1 puede consumir la toxina 2, la toxina 3 y la toxina 4
    #La especie 2 puede consumir la toxina 1, la toxina 3 y la toxina 4
    #La especie 3 puede consumir la toxina 1, la toxina 2 y la toxina 4
    #La especie 4 puede consumir la toxina 1, la toxina 2 y la toxina 3
    if species==1:
        orden=[2,3,4]
    elif species==2:
        orden=[1,3,4]
    ...
    if globals()["toxin_"+str(orden[0])][ag.y,ag.x]>0:
        globals()["toxin_"+str(orden[0])][ag.y,ag.x]-=1
        consume_toxina=True
    elif globals()["toxin_"+str(orden[1])][ag.y,ag.x]>0:
        globals()["toxin_"+str(orden[1])][ag.y,ag.x]-=1
        consume_toxina=True
    elif globals()["toxin_"+str(orden[2])][ag.y,ag.x]>0:
        globals()["toxin_"+str(orden[2])][ag.y,ag.x]-=1
        consume_toxina=True
...
os.mkdir("/home/pedro/Simulaciones_modelo_nulo_competicion_toxicidad")
os.mkdir("/home/pedro/Simulaciones_modelo_nulo_competicion_toxicidad/Distribucion_espacial_0")
os.mkdir("/home/pedro/Simulaciones_modelo_nulo_competicion_toxicidad/Distribucion_espacial_500")
os.mkdir("/home/pedro/Simulaciones_modelo_nulo_competicion_toxicidad/Distribucion_espacial_1000")
os.mkdir("/home/pedro/Simulaciones_modelo_nulo_competicion_toxicidad/Distribucion_espacial_1500")
os.mkdir("/home/pedro/Simulaciones_modelo_nulo_competicion_toxicidad/Distribucion_espacial_2000")
os.mkdir("/home/pedro/Simulaciones_modelo_nulo_competicion_toxicidad/Evolucion_poblaciones")
os.mkdir("/home/pedro/Simulaciones_modelo_nulo_competicion_toxicidad/Coordenadas_espaciales_1000")
os.mkdir("/home/pedro/Simulaciones_modelo_nulo_competicion_toxicidad/Coordenadas_espaciales_2000")
...
#Hay que tener cuidado con los documentos e imagenes no se suporpongan; por ello les asignamos nombres distintos
    ...
    initialize()
    observe()
    os.chdir("/home/pedro/Simulaciones_modelo_nulo_competicion_toxicidad/Distribucion_espacial_0")
    matplotlib.pyplot.savefig("modelo_nulo_competicion_toxicidad_0_"+str(simulacion)+".png")
    ...
    for j in range(number_steps):
        ...
        if j==(number_steps*0.25):
            observe()
            os.chdir("/home/pedro/Simulaciones_modelo_nulo_competicion_toxicidad/Distribucion_espacial_500")
            matplotlib.pyplot.savefig("modelo_nulo_competicion_toxicidad_500_"+str(simulacion)+".png")
            
        if j==(number_steps*0.5):
            
            observe()
            os.chdir("/home/pedro/Simulaciones_modelo_nulo_competicion_toxicidad/Distribucion_espacial_1000")
            matplotlib.pyplot.savefig("modelo_nulo_competicion_toxicidad_1000_"+str(simulacion)+".png")
            
            os.chdir("/home/pedro/Simulaciones_modelo_nulo_competicion_toxicidad/Coordenadas_espaciales_1000")
            document=open("modelo_nulo_competicion_toxicidad_coordenadas_espaciales_1000_"+str(simulacion)+".txt","w")
        ...
        if j==(number_steps*0.75):
            observe()
            os.chdir("/home/pedro/Simulaciones_modelo_nulo_competicion_toxicidad/Distribucion_espacial_1500")
            matplotlib.pyplot.savefig("modelo_nulo_competicion_toxicidad_1500_"+str(simulacion)+".png")
    
    observe()
    os.chdir("/home/pedro/Simulaciones_modelo_nulo_competicion_toxicidad/Distribucion_espacial_2000")
    matplotlib.pyplot.savefig("modelo_nulo_competicion_toxicidad_2000_"+str(simulacion)+".png")
    
    os.chdir("/home/pedro/Simulaciones_modelo_nulo_competicion_toxicidad/Coordenadas_espaciales_2000")
    document=open("modelo_nulo_competicion_toxicidad_coordenadas_espaciales_2000_"+str(simulacion)+".txt","w")
    document.write("Especie" + '\t' + "Coordenadas X" + '\t' + "Coordenadas Y" + '\n')
    for ag in agents:
        document.write(str(ag.type) + '\t' + str(ag.x) + '\t' + str(ag.y) + '\n')
    document.close()

    os.chdir("/home/pedro/Simulaciones_modelo_nulo_competicion_toxicidad/Evolucion_poblaciones")
    document=open("modelo_nulo_competicion_toxicidad_"+str(simulacion)+".txt","w")
