...
def initialize():
    ...
    global agents, public_good_1, public_good_2, public_good_3, public_good_4
    ...
    public_good_3=zeros([rows_columns,rows_columns])
    public_good_4=zeros([rows_columns,rows_columns])
...
def observe():
    ...
    if len(species_1)>0:
        matplotlib.pyplot.scatter([ag.x for ag in species_1],[ag.y for ag in species_1], marker="o",color="#81D4F4")
    if len(species_2)>0:
        matplotlib.pyplot.scatter([ag.x for ag in species_2],[ag.y for ag in species_2], marker="o",color="#2196F3")
    if len(species_3)>0:
        matplotlib.pyplot.scatter([ag.x for ag in species_3],[ag.y for ag in species_3], marker="o",color="#3F51B5")
    if len(species_4)>0:
        matplotlib.pyplot.scatter([ag.x for ag in species_4],[ag.y for ag in species_4], marker="o",color="#1A237E")
    #Dibujamos en la grafica los bienes comunes
    color0 = matplotlib.colors.colorConverter.to_rgba('white')
    color1 = matplotlib.colors.colorConverter.to_rgba('#81D4F4')
    color2 = matplotlib.colors.colorConverter.to_rgba('#2196F3')
    color3 = matplotlib.colors.colorConverter.to_rgba('#3F51B5')
    color4 = matplotlib.colors.colorConverter.to_rgba('#1A237E')                                                      
    ...
    matplotlib.pyplot.imshow(public_good_1,cmap=cmap1,vmin=0,vmax=5,alpha=1)
    matplotlib.pyplot.imshow(public_good_2,cmap=cmap2,vmin=0,vmax=5,alpha=0.75)
    matplotlib.pyplot.imshow(public_good_3,cmap=cmap3,vmin=0,vmax=5,alpha=0.5)
    matplotlib.pyplot.imshow(public_good_4,cmap=cmap4,vmin=0,vmax=5,alpha=0.25)
...
def update():
    ...
    if detection_neighbors>0:
        if random()<secretion_rate:
            if species==1:
                ...
                public_good_3[ag.y,ag.x]+=1  
            elif species==4:
                public_good_4[ag.y,ag.x]+=1                 
    
    #CONSUMO DE BIENES COMUNES Y ALIMENTO
    #Como los arrays espaciales son independientes, en una misma posicion podriamos tener bienes comunes que provienen de especies distintas
    #A nosotros nos interesa que, a cada iteracion, un agente solo consuma un bien comun, de manera que cuando ya ha consumido uno se deja de comprobar si existen de otro tipo
    #Para no establecer un orden de predominancia, hacemos que el orden sea aleatorio
    #La especie 1 puede consumir el bien comun 2, el bien comun 3 y el bien comun 4
    #La especie 2 puede consumir el bien comun 1, el bien comun 3 y el bien comun 4
    #La especie 3 puede consumir el bien comun 1, el bien comun 2 y el bien comun 4
    #La especie 4 puede consumir el bien comun 1, el bien comun 2 y el bien comun 3
    if species==1:
        orden=[2,3,4]
    elif species==2:
        orden=[1,3,4]
    ...
    if globals()["public_good_"+str(orden[0])][ag.y,ag.x]>0:
        globals()["public_good_"+str(orden[0])][ag.y,ag.x]-=1
        consume_bien_comun=True
    elif globals()["public_good_"+str(orden[1])][ag.y,ag.x]>0:
        globals()["public_good_"+str(orden[1])][ag.y,ag.x]-=1
        consume_bien_comun=True
    elif globals()["public_good_"+str(orden[2])][ag.y,ag.x]>0:
        globals()["public_good_"+str(orden[2])][ag.y,ag.x]-=1
        consume_bien_comun=True
    ...
    #Si lo ha consumido, puede reproducirse con mayor probabilidad (se duplica la tasa de reproduccion) y morir
...
os.mkdir("/home/pedro/Simulaciones_modelo_nulo_cooperacion")
os.mkdir("/home/pedro/Simulaciones_modelo_nulo_cooperacion/Distribucion_espacial_0")
os.mkdir("/home/pedro/Simulaciones_modelo_nulo_cooperacion/Distribucion_espacial_500")
os.mkdir("/home/pedro/Simulaciones_modelo_nulo_cooperacion/Distribucion_espacial_1000")
os.mkdir("/home/pedro/Simulaciones_modelo_nulo_cooperacion/Distribucion_espacial_1500")
os.mkdir("/home/pedro/Simulaciones_modelo_nulo_cooperacion/Distribucion_espacial_2000")
os.mkdir("/home/pedro/Simulaciones_modelo_nulo_cooperacion/Evolucion_poblaciones")
os.mkdir("/home/pedro/Simulaciones_modelo_nulo_cooperacion/Coordenadas_espaciales_1000")
os.mkdir("/home/pedro/Simulaciones_modelo_nulo_cooperacion/Coordenadas_espaciales_2000")
...
#Hay que tener cuidado con los documentos e imagenes no se suporpongan; por ello les asignamos nombres distintos
    ...
    initialize()
    observe()
    os.chdir("/home/pedro/Simulaciones_modelo_nulo_cooperacion/Distribucion_espacial_0")
    matplotlib.pyplot.savefig("modelo_nulo_cooperacion_0_"+str(simulacion)+".png")
    ...
    for j in range(number_steps):
        ...
        if j==(number_steps*0.25):
            observe()
            os.chdir("/home/pedro/Simulaciones_modelo_nulo_cooperacion/Distribucion_espacial_500")
            matplotlib.pyplot.savefig("modelo_nulo_cooperacion_500_"+str(simulacion)+".png")
            
        if j==(number_steps*0.5):
            
            observe()
            os.chdir("/home/pedro/Simulaciones_modelo_nulo_cooperacion/Distribucion_espacial_1000")
            matplotlib.pyplot.savefig("modelo_nulo_cooperacion_1000_"+str(simulacion)+".png")
            
            os.chdir("/home/pedro/Simulaciones_modelo_nulo_cooperacion/Coordenadas_espaciales_1000")
            document=open("modelo_nulo_cooperacion_coordenadas_espaciales_1000_"+str(simulacion)+".txt","w")
        ...
        if j==(number_steps*0.75):
            observe()
            os.chdir("/home/pedro/Simulaciones_modelo_nulo_cooperacion/Distribucion_espacial_1500")
            matplotlib.pyplot.savefig("modelo_nulo_cooperacion_1500_"+str(simulacion)+".png")
    
    observe()
    os.chdir("/home/pedro/Simulaciones_modelo_nulo_cooperacion/Distribucion_espacial_2000")
    matplotlib.pyplot.savefig("modelo_nulo_cooperacion_2000_"+str(simulacion)+".png")
    
    os.chdir("/home/pedro/Simulaciones_modelo_nulo_cooperacion/Coordenadas_espaciales_2000")
    document=open("modelo_nulo_cooperacion_coordenadas_espaciales_2000_"+str(simulacion)+".txt","w")
    document.write("Especie" + '\t' + "Coordenadas X" + '\t' + "Coordenadas Y" + '\n')
    for ag in agents:
        document.write(str(ag.type) + '\t' + str(ag.x) + '\t' + str(ag.y) + '\n')
    document.close()

    os.chdir("/home/pedro/Simulaciones_modelo_nulo_cooperacion/Evolucion_poblaciones")
    document=open("modelo_nulo_cooperacion_"+str(simulacion)+".txt","w")
