#MODELO COMPETICION INHIBICION (4 ESPECIES)
...
def initialize():
    ...
    global agents, inhibitory_1, inhibitory_2, inhibitory_3, inhibitory_4
    ...
    for i in range (number_agents*number_species):
        ...
        agents.append(ag)  
    #Generamos arrays para las sustancias inhibidoras
    inhibitory_1=zeros([rows_columns,rows_columns])
    inhibitory_2=zeros([rows_columns,rows_columns])
    inhibitory_3=zeros([rows_columns,rows_columns])
    inhibitory_4=zeros([rows_columns,rows_columns])
...
def observe():
    ...
    if len(species_1)>0:
        matplotlib.pyplot.scatter([ag.x for ag in species_1],[ag.y for ag in species_1], marker="o",color="#FFCDD2")
    if len(species_2)>0:
        matplotlib.pyplot.scatter([ag.x for ag in species_2],[ag.y for ag in species_2], marker="o",color="#E57373")
    if len(species_3)>0:
        matplotlib.pyplot.scatter([ag.x for ag in species_3],[ag.y for ag in species_3], marker="o",color="#E53935")
    if len(species_4)>0:
        matplotlib.pyplot.scatter([ag.x for ag in species_4],[ag.y for ag in species_4], marker="o",color="#B71C1C")
    #Dibujamos en la grafica las sustancias inhibidoras
    color0 = matplotlib.colors.colorConverter.to_rgba('white')
    color1 = matplotlib.colors.colorConverter.to_rgba('#FFCDD2')
    color2 = matplotlib.colors.colorConverter.to_rgba('#E57373')
    color3 = matplotlib.colors.colorConverter.to_rgba('#E53935')
    color4 = matplotlib.colors.colorConverter.to_rgba('#B71C1C')                                                      
    ...
    matplotlib.pyplot.imshow(inhibitory_1,cmap=cmap1,vmin=0,vmax=5,alpha=1)
    matplotlib.pyplot.imshow(inhibitory_2,cmap=cmap2,vmin=0,vmax=5,alpha=0.75)
    matplotlib.pyplot.imshow(inhibitory_3,cmap=cmap3,vmin=0,vmax=5,alpha=0.5)
    matplotlib.pyplot.imshow(inhibitory_4,cmap=cmap4,vmin=0,vmax=5,alpha=0.25)
...
def update():
    ...
    #En este codigo vamos a introducir la idea de secrecion por estimulo; solo se secretan sustancias inhibidoras cuando la bacteria sensa que esta rodeada de bacterias de la otra especie
    ...
    if detection_neighbors>0:
        if random()<secretion_rate:
            if species==1:
                ...
                inhibitory_3[ag.y,ag.x]+=1  
            elif species==4:
                inhibitory_4[ag.y,ag.x]+=1                 
    
    #CONSUMO DE SUSTANCIAS INHIBIDORAS Y ALIMENTO
    #Como los arrays espaciales son independientes, en una misma posicion podriamos tener sustancias inhibidoras que provienen de especies distintas
    #A nosotros nos interesa que, a cada iteracion, un agente solo consuma un inhibidor, de manera que cuando ya ha consumido uno se deja de comprobar si existen de otro tipo
    #Para no establecer un orden de predominancia, hacemos que el orden sea aleatorio
    #La especie 1 puede consumir el inhibidor 2, el inhibidor 3 y el inhibidor 4
    #La especie 2 puede consumir el inhibidor 1, el inhibidor 3 y el inhibidor 4
    #La especie 3 puede consumir el inhibidor 1, el inhibidor 2 y el inhibidor 4
    #La especie 4 puede consumir el inhibidor 1, el inhibidor 2 y el inhibidor 3
    if species==1:
        orden=[2,3,4]
    elif species==2:
        orden=[1,3,4]
    ...
    if globals()["inhibitory_"+str(orden[0])][ag.y,ag.x]>0:
        globals()["inhibitory_"+str(orden[0])][ag.y,ag.x]-=1
        consume_inhibidor=True
    elif globals()["inhibitory_"+str(orden[1])][ag.y,ag.x]>0:
        globals()["inhibitory_"+str(orden[1])][ag.y,ag.x]-=1
        consume_inhibidor=True
    elif globals()["inhibitory_"+str(orden[2])][ag.y,ag.x]>0:
        globals()["inhibitory_"+str(orden[2])][ag.y,ag.x]-=1
        consume_inhibidor=True
    ...
    #El agente puede haber consumido el inhibidor o no haberlo hecho
    #Si lo ha consumido, puede reproducirse con menor probabilidad (se divide por dos la tasa de reproduccion) y morir
    #Si no lo ha consumido, puede reproducirse y morir
    if consume_inhibidor==True:
...
os.mkdir("/home/pedro/Simulaciones_modelo_nulo_competicion_inhibicion")
os.mkdir("/home/pedro/Simulaciones_modelo_nulo_competicion_inhibicion/Distribucion_espacial_0")
os.mkdir("/home/pedro/Simulaciones_modelo_nulo_competicion_inhibicion/Distribucion_espacial_500")
os.mkdir("/home/pedro/Simulaciones_modelo_nulo_competicion_inhibicion/Distribucion_espacial_1000")
os.mkdir("/home/pedro/Simulaciones_modelo_nulo_competicion_inhibicion/Distribucion_espacial_1500")
os.mkdir("/home/pedro/Simulaciones_modelo_nulo_competicion_inhibicion/Distribucion_espacial_2000")
os.mkdir("/home/pedro/Simulaciones_modelo_nulo_competicion_inhibicion/Evolucion_poblaciones")
os.mkdir("/home/pedro/Simulaciones_modelo_nulo_competicion_inhibicion/Coordenadas_espaciales_1000")
os.mkdir("/home/pedro/Simulaciones_modelo_nulo_competicion_inhibicion/Coordenadas_espaciales_2000")
...
#Hay que tener cuidado con los documentos e imagenes no se suporpongan; por ello les asignamos nombres distintos
    ...
    initialize()
    observe()
    os.chdir("/home/pedro/Simulaciones_modelo_nulo_competicion_inhibicion/Distribucion_espacial_0")
    matplotlib.pyplot.savefig("modelo_nulo_competicion_inhibicion_0_"+str(simulacion)+".png")
    ...
    for j in range(number_steps):
        ...
        if j==(number_steps*0.25):
            observe()
            os.chdir("/home/pedro/Simulaciones_modelo_nulo_competicion_inhibicion/Distribucion_espacial_500")
            matplotlib.pyplot.savefig("modelo_nulo_competicion_inhibicion_500_"+str(simulacion)+".png")
            
        if j==(number_steps*0.5):
            
            observe()
            os.chdir("/home/pedro/Simulaciones_modelo_nulo_competicion_inhibicion/Distribucion_espacial_1000")
            matplotlib.pyplot.savefig("modelo_nulo_competicion_inhibicion_1000_"+str(simulacion)+".png")
            
            os.chdir("/home/pedro/Simulaciones_modelo_nulo_competicion_inhibicion/Coordenadas_espaciales_1000")
            document=open("modelo_nulo_competicion_inhibicion_coordenadas_espaciales_1000_"+str(simulacion)+".txt","w")
        ...
        if j==(number_steps*0.75):
            observe()
            os.chdir("/home/pedro/Simulaciones_modelo_nulo_competicion_inhibicion/Distribucion_espacial_1500")
            matplotlib.pyplot.savefig("modelo_nulo_competicion_inhibicion_1500_"+str(simulacion)+".png")
    
    observe()
    os.chdir("/home/pedro/Simulaciones_modelo_nulo_competicion_inhibicion/Distribucion_espacial_2000")
    matplotlib.pyplot.savefig("modelo_nulo_competicion_inhibicion_2000_"+str(simulacion)+".png")
    
    os.chdir("/home/pedro/Simulaciones_modelo_nulo_competicion_inhibicion/Coordenadas_espaciales_2000")
    document=open("modelo_nulo_competicion_inhibicion_coordenadas_espaciales_2000_"+str(simulacion)+".txt","w")
    document.write("Especie" + '\t' + "Coordenadas X" + '\t' + "Coordenadas Y" + '\n')
    for ag in agents:
        document.write(str(ag.type) + '\t' + str(ag.x) + '\t' + str(ag.y) + '\n')
    document.close()

    os.chdir("/home/pedro/Simulaciones_modelo_nulo_competicion_inhibicion/Evolucion_poblaciones")
    document=open("modelo_nulo_competicion_inhibicion_"+str(simulacion)+".txt","w")
